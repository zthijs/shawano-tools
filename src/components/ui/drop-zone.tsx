...
import { CropDialog } from "./crop-modal";
	...
	({ className, children, ...props }, forwardedRef) => {
		...
		return (
			<div
				...
				aria-label="dropzone"
				className={cn(
					"flex items-center justify-center rounded-xl border border-input bg-background px-4 py-2 ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
...
export interface DropzoneFileListContext<TUploadRes, TUploadError> {
...
const DropzoneResizeFile = forwardRef<
	HTMLButtonElement,
	DropzoneRemoveFileProps
>(({ className, ...props }, ref) => {
	const context = useDropzoneFileListContext();
	...
	return (
		<CropDialog
			file={context.fileStatus.result as string}
			context={context}
			trigger={
				<Button
					ref={ref}
				...
				>
					{props.children}
					<span className="sr-only">Resize file</span>
				</Button>
			}
		/>
	);
});
DropzoneResizeFile.displayName = "DropzoneResizeFile";
